...
4. **比较**：按配方计算质量分组均值、极差、95% t 置信区间；在覆盖完整时计算四因子
   ...
   对齐）、无空缺区组格、每配方有有效数据，以及 **区组内覆盖**——每个配方在每个区组中
   都要覆盖每个因子的全部设计水平（主效应）和每对因子的全部交互单元（二阶交互）。
   只在跨区组汇总后才“凑齐”的水平（如短洗全在区组1、长洗全在区组2）属于因子与区组
   混杂，判为覆盖不完整。任一闸门不满足即 `inconclusive`，不输出无法成立的效应值、
